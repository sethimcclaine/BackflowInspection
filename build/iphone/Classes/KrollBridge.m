...
NSString * BackflowInspection$ModuleRequireFormat = @"(function(exports){"
...
@implementation BackflowInspectionObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_backflowinspection);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create BackflowInspection global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "BackflowInspection" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_backflowinspection = [[BackflowInspectionObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_backflowinspection];
	
	NSString *_backflowinspectionNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _backflowinspectionNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_backflowinspection valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:BackflowInspection$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.backflowinspection.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.backflowinspection.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
