...
//Rare exceptions to indicate a bug in the _backflowinspection code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_backflowinspection = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_backflowinspection]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.backflowinspection.module"
